...
@section('main')
    ...
    <div class="container">
        ...
        <div class="form-div ">
            ...
            <form method="POST" action="{{ route('updateservicesname',['id'=>$servicename->id]) }}">

                @csrf
                <div class="row">
                    <div class="col-md-6 form-group mb-2">
                        <label class="form-label" for="service_name">Service Name</label>
                        <input required value="{{$servicename->service_name}}" class="form-control @error('service_name') is-invalid @enderror" name="service_name"
                    ...
                    <div class="col-md-6 form-group mb-2">
                        ...
                        <select required class="form-control @error('category_id') is-invalid @enderror" name="category_id"
                            ...
                            <option value="{{ $servicename->category->id }}" selected>
                                {{ $servicename->category->category_name }}</option>
                            @foreach ($categories as $data)
                                @if ($data->id !== $servicename->category->id)
                    ...
                    <div class="col-md-6 form-group mb-2">
                        ...
                        <select required class="form-control @error('niche_id') is-invalid @enderror" name="niche_id"
                            id="niche_id">
                            <option value="{{ $servicename->niche->id }}" selected>{{ $servicename->niche->niche_name }}</option>
                            @foreach ($niches as $data)
                                @if ($data->id !== $servicename->niche->id) <!-- Exclude the selected option -->
                                    <option value="{{ $data->id }}">{{ $data->niche_name }}</option>
                                @endif
                            @endforeach
